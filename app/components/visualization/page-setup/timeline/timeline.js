...
export default Component.extend(Evented, {
  ...
  cleanup() {
    ...
    this.get('listeners').forEach(([serviceName, event, listenerFunction]) => {
      this.get(serviceName).off(event, listenerFunction);
    });
  ...
  initListener() {
    ...
    // start subscriptions
    this.get('listeners').forEach(([serviceName, event, listenerFunction]) => {
      this.get(serviceName).on(event, listenerFunction);
    });
  },
  ...
  renderChart() {
    ...
    const ctx = $('#timelineCanvas').get(0).getContext('2d');
  ...
  chartClickHandler(evt) {
    ...
    const activePoint = timelineChart.getElementAtEvent(evt)[0];
    ...
    if (activePoint) {
      ...
      const elementIndex = activePoint._index;
      /* eslint-enable no-underscore-dangle */
      const retrievedTimestamp = data.datasets[datasetIndex].data[elementIndex].x;
      ...
      if (lastHighlightedElementIndex === elementIndex) {
        ...
        if (lastHighlightedElementIndex) {
          timelineChart.getDatasetMeta(datasetIndex)
            .data[lastHighlightedElementIndex].custom = colorsDefault;
        }
    ...
    } else if (lastHighlightedElementIndex) {
  ...
  updateChart(newTimestamp) {
    ...
    if (lastHighlightedElementIndex) {
      updatedTimelineChart.getDatasetMeta(0)
        .data[lastHighlightedElementIndex].custom = colorsDefault;
