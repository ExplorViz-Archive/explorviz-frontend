...
export default class ConfigurationOverview extends Component<IArgs> {
    ...
    toggleConfigStatus(id: string) {
        ...
        status ? this.configRepo.deactiveConfiguration(landscapeToken, id) : this.configRepo.activateConfiguration(landscapeToken, id);
    }
    ...
    changeConfigName(configuration: ConfigurationItem, value : any) {
        ...
        try {
            ...
            const update = {id: configuration.id, name: name, key: configuration.key, color: configuration.color, status: configuration.status};
    ...
    changeConfigKey(configuration: ConfigurationItem, value : any) {
        ...
        try {
            ...
            const update = {id: configuration.id, name: configuration.name, key: key, color: configuration.color, status: configuration.status};
            this.configRepo.editConfiguration(landscapeToken, update);
        } catch(e) {
            // Selection failed ?
        }
    ...
    changeConfigColor(configuration: ConfigurationItem, value: any) {
        ...
        try {
            ...
            const update = {id: configuration.id, name: configuration.name, key: configuration.key, color: color, status: configuration.status};
            this.configRepo.editConfiguration(landscapeToken, update);
        } catch(e) {
            // Selection failed ?
        }
