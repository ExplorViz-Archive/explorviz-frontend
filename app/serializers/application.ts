...
interface ILinks { first: string; last: string; next: string; prev: string; }
...
export default class ApplicationSerializer extends JSONAPISerializer {
  ...
  // @override
  normalizeQueryRecordResponse(store: DS.Store, primaryModelClass: DS.Model, payload: {},
    id: string | number, requestType: string) {
  ...
  normalizeQueryResponse(store: DS.Store, primaryModelClass: DS.Model, payload: any,
    id: string | number, requestType: string) {
    const result: any = super.normalizeQueryResponse(store, primaryModelClass,
  ...
  // see: https://emberigniter.com/pagination-in-ember-with-json-api-backend/
  createPageMeta(data: ILinks) {
    const meta: any = {};

    Object.keys(data).forEach((type: keyof ILinks) => {
